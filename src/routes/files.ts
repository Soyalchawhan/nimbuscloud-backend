...
// ── Helper: save file to local disk ──────────────────────────────────────────
...
const getLocalFilePath = (storageKey: string): string => {
  return path.join(process.cwd(), 'uploads', storageKey.replace(/\//g, path.sep));
};

// ── PUT /api/files/upload-part/:storageKey (NO AUTH - public upload endpoint) ─
  ...
  try {
    ...
    req.on('end', () => {
      try {
        ...
        res.status(500).json({ error: { code: 'WRITE_ERROR', message: 'Failed to save file' } });
      }
    });
...
// ── GET /api/files/download/:storageKey (NO AUTH - public download) ───────────
...
filesRouter.post('/init', async (req: Request, res: Response) => {
  ...
  const fileId = uuidv4();
  const storageKey = generateStorageKey(req.user!.id, fileId, body.name, body.folderId);

  const { rows } = await query<File>(
    `INSERT INTO files (id, name, mime_type, size_bytes, storage_key, owner_id, folder_id, status)
     VALUES ($1, $2, $3, $4, $5, $6, $7, 'uploading')
     RETURNING id, name, mime_type as "mimeType", size_bytes as "sizeBytes",
               storage_key as "storageKey", owner_id as "ownerId", folder_id as "folderId",
...
filesRouter.post('/complete', async (req: Request, res: Response) => {
  try {
    ...
    if (fileRows[0].ownerId !== req.user!.id) {
    ...
    try {
      const { rows: versionRows } = await query(
        `INSERT INTO file_versions
         ...
         RETURNING id`,
        [body.fileId, fileRows[0].storageKey, fileRows[0].sizeBytes, req.user!.id]
      );
    ...
    try {
      await logActivity(req.user!.id, 'upload', 'file', body.fileId, { name: rows[0].name });
    } catch {}
...
filesRouter.get('/:id', async (req: Request, res: Response) => {
  ...
  const { rows } = await query<File>(
    `SELECT id, name, mime_type as "mimeType", size_bytes as "sizeBytes",
            storage_key as "storageKey", owner_id as "ownerId", folder_id as "folderId",
            status, checksum, created_at as "createdAt", updated_at as "updatedAt"
  ...
  const signedUrl = `http://localhost:${process.env.PORT || 8080}/api/files/download/${encodeURIComponent(rows[0].storageKey)}`;
...
filesRouter.patch('/:id', async (req: Request, res: Response) => {
  ...
  const { rows } = await query<File>(
    `UPDATE files
     ...
     RETURNING id, name, mime_type as "mimeType", size_bytes as "sizeBytes",
               folder_id as "folderId", status, created_at as "createdAt", updated_at as "updatedAt"`,
  ...
  if (body.name) await logActivity(req.user!.id, 'rename', 'file', req.params.id, { name: body.name });
  if ('folderId' in body) await logActivity(req.user!.id, 'move', 'file', req.params.id);
