...
sharesRouter.post('/', async (req: Request, res: Response) => {
  ...
  const { rows } = await query(
    `INSERT INTO shares
     ...
     RETURNING id, resource_type as "resourceType", resource_id as "resourceId",
               grantee_user_id as "granteeUserId", role,
  ...
  await logActivity(
    ...
    body.resourceType,
    body.resourceId,
    { granteeUserId: body.granteeUserId, role: body.role }
  );
...
sharesRouter.get('/:resourceType/:resourceId', async (req: Request, res: Response) => {
  ...
  const { rows } = await query(
    `SELECT s.id, s.resource_type as "resourceType", s.resource_id as "resourceId",
            s.grantee_user_id as "granteeUserId", s.role,
            s.created_at as "createdAt",
            u.name as "granteeName", u.email as "granteeEmail",
            u.image_url as "granteeImageUrl"
     FROM shares s
